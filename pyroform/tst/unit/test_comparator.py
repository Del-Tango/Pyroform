...
def minimal_system_state():
    ...
    return {"users": [], "groups": [], "mounted_devices": []}
...
#       """Test comparison with device configuration."""
    ...
    def test_compare_states_basic(
        self, comparator, pyro_config, minimal_system_state, mock_stdout
    ):
    ...
    def test_compare_states_empty_difference_categories_removed(
        self, comparator, mock_stdout
    ):
        """Test that empty difference categories are removed from output."""
        system_state = {"users": [], "groups": [], "mounted_devices": []}
    ...
    def test_compare_states_all_config_none(
        self, comparator, pyro_config, system_state
    ):
    ...
    def test_compare_states_debug_output(
        self, comparator, mock_stdout, system_state, pyro_config
    ):
...
#       """Test comparison when config_users is None."""
    ...
    def test_compare_users_with_excludes(self, comparator, mock_user, mock_excludes):
        ...
        comparator._compare_users(
            config_users, current_users, differences, mock_excludes
        )
    ...
    def test_check_user_compliance_exists(self, comparator, mock_user):
        ...
        with patch.object(comparator, "_check_user_properties") as mock_check:
            ...
            mock_check.assert_called_once_with(
                mock_user, current_users["testuser"], differences
            )
    ...
    def test_find_extra_users(self, comparator):
        ...
        current_users = {
            ...
            "user3": {
                "username": "user3",
                "uid": 1002,
                "home_directory": "/home/user3",
            },
            "user4": {
                "username": "user4",
                "uid": 1003,
                "home_directory": "/home/user4",
            },
        }
        differences = comparator._initialize_differences_dict()

        comparator._find_extra_users(
            config_user_names, current_users, differences, None
        )
    ...
    def test_find_extra_users_with_excludes(self, comparator, mock_excludes):
        ...
        comparator._find_extra_users(
            config_user_names, current_users, differences, mock_excludes
        )
...
class TestGroupComparison:
    ...
    def test_compare_groups_extra_group(self, comparator):
        ...
        config_groups = []
        current_groups = [{"groupname": "extra_group", "gid": 1001, "members": []}]
        differences = comparator._initialize_differences_dict()
    ...
    def test_compare_groups_with_excludes(self, comparator, mock_group, mock_excludes):
        ...
        comparator._compare_groups(
            config_groups, current_groups, differences, mock_excludes
        )
...
#       """Test basic filesystem comparison."""
    ...
    def test_check_mountpoint_missing(self, comparator, mock_device):
        ...
        current_fs_state = {"/path1": {"mountpoint": "/", "type": "directory"}}
    ...
    def test_check_device_contents_excluded(self, comparator, mock_device):
        ...
        with patch.object(
            comparator, "_parse_config_state_entry"
        ) as mock_parse, patch.object(
            comparator, "_should_exclude"
...
class TestFindExtraFilesystemItems:
    ...
    def test_find_extra_filesystem_items_empty(self, comparator):
        ...
        comparator._find_extra_filesystem_items(
            config_paths, current_fs_state, differences
        )
...
#       """Test when exclude set is empty."""
    ...
    def test_should_exclude_user(self, comparator, mock_excludes):
        ...
        assert (
            comparator._should_exclude("excluded_user", mock_excludes, "users") == True
        )
        assert (
            comparator._should_exclude("regular_user", mock_excludes, "users") == False
        )

    def test_should_exclude_group(self, comparator, mock_excludes):
        """Test excluding groups."""
        assert (
            comparator._should_exclude("excluded_group", mock_excludes, "groups")
            == True
        )
        assert (
            comparator._should_exclude("regular_group", mock_excludes, "groups")
            == False
        )
    ...
    def test_should_exclude_device(self, comparator, mock_excludes):
        ...
        assert (
            comparator._should_exclude("/dev/excluded", mock_excludes, "devices")
            == True
        )
        assert (
            comparator._should_exclude("/dev/regular", mock_excludes, "devices")
            == False
        )
    ...
    def test_should_exclude_file(self, comparator, mock_excludes):
        ...
        assert (
            comparator._should_exclude("/etc/excluded.txt", mock_excludes, "fl") == True
        )
        assert (
            comparator._should_exclude("/etc/regular.txt", mock_excludes, "fl") == False
        )

    def test_should_exclude_link(self, comparator, mock_excludes):
        """Test excluding links."""
        assert (
            comparator._should_exclude("/etc/link_excluded", mock_excludes, "ln")
            == True
        )
        assert (
            comparator._should_exclude("/etc/link_regular", mock_excludes, "ln")
            == False
        )
...
#       """Test building filesystem state with None entries."""
    ...
    def test_compare_states_missing_keys(self, comparator, pyro_config):
        ...
        system_state = {"users": []}  # Missing 'groups' and 'mounted_devices'
    ...
    def test_compare_states_none_values(self, comparator, pyro_config):
        ...
        system_state = {"users": None, "groups": None, "mounted_devices": None}
