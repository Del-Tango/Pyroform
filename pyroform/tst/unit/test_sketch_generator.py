...
from pyroform.src.sketch_generator import (
    SketchGenerator, CommandGenerator, ExclusionChecker, StateEntryParser
)
from pyroform.src.models import (
    PyroConfig, User, Group, Device, Exclude, ActionType
)
...
#       """Test mount command generation without partition."""
    ...
    def test_init(self, mock_stdout):
        """Test initialization."""
        generator = CommandGenerator(dry_run=True, stdout=mock_stdout)

        assert generator.dry_run == True
        assert generator.cmd_prefix == '# '
        assert generator.stdout == mock_stdout
    ...
    def test_generate_mountpoint_command(self, mock_device):
        ...
        assert command["name"] == f"Creating System Mountpoint Directory {mock_device.mountpoint}"
    ...
    def test_command_structure_consistency(self, mock_user):
        ...
        required_fields = ["name", "cmd", "setup-cmd", "on-ok-cmd", "on-nok-cmd", "fatal-nok"]
...
class TestExclusionChecker:
    ...
    def test_should_exclude_user(self, mock_stdout):
        ...
        # Should exclude when in list
        assert checker.should_exclude_user("excluded_user", ["excluded_user", "other"]) == True
        mock_stdout.info.assert_called_with("Excluding system user: excluded_user")

        # Should not exclude when not in list
        mock_stdout.info.reset_mock()
        assert checker.should_exclude_user("regular_user", ["excluded_user"]) == False
    ...
    def test_should_exclude_group(self, mock_stdout):
        ...
        assert checker.should_exclude_group("excluded_group", ["excluded_group"]) == True
        mock_stdout.info.assert_called_with("Excluding system group: excluded_group")

        mock_stdout.info.reset_mock()
        assert checker.should_exclude_group("regular_group", ["excluded_group"]) == False
        mock_stdout.info.assert_not_called()
    ...
    def test_should_exclude_device(self, mock_stdout):
        ...
        assert checker.should_exclude_device("/dev/excluded", ["/dev/excluded"]) == True
        mock_stdout.info.assert_called_with("Excluding block storage device: /dev/excluded")

        assert checker.should_exclude_device("/dev/regular", ["/dev/excluded"]) == False

    def test_should_exclude_path(self, mock_stdout):
        """Test path exclusion check."""
        checker = ExclusionChecker(stdout=mock_stdout)

        assert checker.should_exclude_path("/excluded/path", ["/excluded/path"]) == True
        mock_stdout.info.assert_called_with("Excluding path: /excluded/path")

        assert checker.should_exclude_path("/regular/path", ["/excluded/path"]) == False
    ...
    def test_has_excluded_parent_nested(self, mock_stdout):
        ...
        assert checker.has_excluded_parent("/excluded/nested/deep", excluded_paths) == True
    ...
    def test_has_excluded_parent_with_path_objects(self, mock_stdout):
        ...
        assert checker.has_excluded_parent("/excluded/child", excluded_paths) == True
        assert checker.has_excluded_parent(Path("/excluded/child"), excluded_paths) == True
...
class TestSketchGeneratorInitialization:
    ...
    def test_init_defaults(self, mock_stdout):
        ...
        assert generator.stdout == mock_stdout
        assert generator.dry_run == False
        assert generator.chunk_size == 500
        assert generator.command_generator is not None
        assert generator.exclusion_checker is not None
        assert generator.state_parser is not None
        assert generator.list_splitter is not None
    ...
    def test_init_components(self, mock_stdout):
        ...
        assert isinstance(generator.command_generator, CommandGenerator)
        assert generator.command_generator.dry_run == False

        assert isinstance(generator.exclusion_checker, ExclusionChecker)
        assert generator.exclusion_checker.stdout == mock_stdout

        assert isinstance(generator.state_parser, StateEntryParser)
        assert isinstance(generator.list_splitter, ListSplitter)
        assert generator.list_splitter.chunk_size == 500

        assert isinstance(generator.validator, SystemValidator)
    ...
    def test_init_dry_run_propagated(self, mock_stdout):
        ...
        assert generator.command_generator.dry_run == True
        assert generator.command_generator.cmd_prefix == '# '
...
class TestGenerateSketch:
    ...
    def test_generate_sketch_validate_error(self, mock_stdout, mock_pyro_config):
        ...
        with pytest.raises(ValueError, match="Validate action does not generate sketches"):
...
#       """Test device state snapshot building with missing fields."""
    ...
    def test_build_device_snapshot(self, mock_stdout, system_state):
        ...
        result = generator._build_device_snapshot(system_state)

        assert isinstance(result, list)
        assert len(result) == 1
        device_snapshot = result[0]
        assert device_snapshot["label"] == "Snapshot of /dev/sda1"
        assert device_snapshot["Path"] == "/dev/sda1"
        assert device_snapshot["Mountpoint"] == "/"
        assert "State" in device_snapshot
        assert isinstance(device_snapshot["State"], list)

    def test_build_device_state_snapshot(self, mock_stdout, system_state):
        """Test device state snapshot building."""
        generator = SketchGenerator(stdout=mock_stdout)
        device = system_state["mounted_devices"][0]

        result = generator._build_device_state_snapshot(device)

        assert isinstance(result, list)
        assert len(result) == 3  # One dir, one file, one symlink

        # Check each entry format
        for entry in result:
        ...
        # Find symlink entry
        symlink_entry = next(e for e in result if e.startswith("ln"))
        assert "UTC" in symlink_entry  # Should contain target
...
#       """Test scorch sketch generation."""
    ...
    def test_generate_cleanup_commands(self, mock_stdout, mock_pyro_config):
        ...
        with patch.object(generator, '_generate_user_cleanup_commands') as mock_users, \
             patch.object(generator, '_generate_group_cleanup_commands') as mock_groups, \
             patch.object(generator, '_generate_filesystem_cleanup_commands') as mock_fs:
    ...
    def test_generate_user_cleanup_commands_no_extra(self, mock_stdout, mock_pyro_config):
    ...
    def test_generate_user_cleanup_commands_with_exclusions(self, mock_stdout, mock_pyro_config):
        ...
        with patch.object(generator.exclusion_checker, 'should_exclude_user') as mock_exclude:
            ...
            with patch.object(generator.list_splitter, 'split') as mock_split:
                ...
                with patch.object(generator.command_generator, 'generate_cleanup_user_command') as mock_cmd:
                    mock_cmd.return_value = {"cmd": "cleanup"}

                    result = generator._generate_user_cleanup_commands(mock_pyro_config, compared)

                    assert len(result) == 1
                    mock_exclude.assert_any_call("excluded_user", mock_pyro_config.excludes.users)
                    mock_exclude.assert_any_call("regular_user", mock_pyro_config.excludes.users)

    def test_generate_user_cleanup_commands_chunking(self, mock_stdout, mock_pyro_config):
        ...
        with patch.object(generator.exclusion_checker, 'should_exclude_user', return_value=False):
            with patch.object(generator.command_generator, 'generate_cleanup_user_command') as mock_cmd:
                mock_cmd.return_value = {"cmd": "cleanup"}
    ...
    def test_generate_group_cleanup_commands(self, mock_stdout, mock_pyro_config):
        ...
        with patch.object(generator.exclusion_checker, 'should_exclude_group', return_value=False):
            with patch.object(generator.command_generator, 'generate_cleanup_group_command') as mock_cmd:
                mock_cmd.return_value = {"cmd": "cleanup group"}

                result = generator._generate_group_cleanup_commands(mock_pyro_config, compared)
    ...
    def test_generate_directory_cleanup_commands(self, mock_stdout, mock_pyro_config):
        ...
        with patch.object(generator.exclusion_checker, 'should_exclude_path', return_value=False), \
             patch.object(generator.exclusion_checker, 'has_excluded_parent', return_value=False), \
             patch.object(generator.command_generator, 'generate_cleanup_directories_command') as mock_cmd:

            mock_cmd.return_value = {"cmd": "cleanup dir"}

            result = generator._generate_directory_cleanup_commands(mock_pyro_config, compared)

            assert len(result) == 1
            mock_cmd.assert_called_once()

    def test_generate_directory_cleanup_commands_excluded_parent(self, mock_stdout, mock_pyro_config):
        ...
        with patch.object(generator.exclusion_checker, 'should_exclude_path', return_value=False), \
             patch.object(generator.exclusion_checker, 'has_excluded_parent', return_value=True):
    ...
    def test_generate_file_cleanup_commands(self, mock_stdout, mock_pyro_config):
        ...
        with patch.object(generator.exclusion_checker, 'should_exclude_path', return_value=False), \
             patch.object(generator.exclusion_checker, 'has_excluded_parent', return_value=False), \
             patch.object(generator.command_generator, 'generate_cleanup_files_command') as mock_cmd:

            mock_cmd.return_value = {"cmd": "cleanup files"}

            result = generator._generate_file_cleanup_commands(mock_pyro_config, compared)
    ...
    def test_generate_filesystem_cleanup_commands(self, mock_stdout, mock_pyro_config):
        ...
        with patch.object(generator, '_generate_directory_cleanup_commands') as mock_dirs, \
             patch.object(generator, '_generate_file_cleanup_commands') as mock_files:

            mock_dirs.return_value = [{"dir": "cleanup"}]
            mock_files.return_value = [{"file": "cleanup"}]

            result = generator._generate_filesystem_cleanup_commands(mock_pyro_config, compared)
...
#       """Test filesystem entry that should be excluded."""
    ...
    def test_generate_configure_sketch(self, mock_stdout, mock_pyro_config):
        ...
        with patch.object(generator, '_generate_user_commands') as mock_users, \
             patch.object(generator, '_generate_group_commands') as mock_groups, \
             patch.object(generator, '_generate_device_commands') as mock_devices:
            ...
            assert result["name"] == f"Pyroform Auto-Generated Sketch {mock_pyro_config.label}"
    ...
    def test_generate_configure_sketch_empty_sections(self, mock_stdout, empty_pyro_config):
        """Test configure sketch with empty sections."""
        generator = SketchGenerator(stdout=mock_stdout)

        with patch.object(generator, '_generate_user_commands', return_value=[]), \
             patch.object(generator, '_generate_group_commands', return_value=[]), \
             patch.object(generator, '_generate_device_commands', return_value=[]):
    ...
    def test_generate_user_commands(self, mock_stdout, mock_user):
        ...
        with patch.object(generator.exclusion_checker, 'should_exclude_user', return_value=False), \
             patch.object(generator.command_generator, 'generate_user_command') as mock_cmd:
    ...
    def test_generate_user_commands_excluded(self, mock_stdout, mock_user):
        ...
        with patch.object(generator.exclusion_checker, 'should_exclude_user', return_value=True):
            result = generator._generate_user_commands([mock_user], ["excluded_user"])
    ...
    def test_generate_group_commands(self, mock_stdout, mock_group):
        ...
        with patch.object(generator.exclusion_checker, 'should_exclude_group', return_value=False), \
             patch.object(generator.command_generator, 'generate_group_command') as mock_cmd:

            mock_cmd.return_value = {"cmd": "groupadd"}

            result = generator._generate_group_commands([mock_group], ["excluded_group"])
    ...
    def test_generate_mount_commands(self, mock_stdout, mock_device):
        ...
        with patch.object(generator.exclusion_checker, 'should_exclude_device', return_value=False), \
             patch.object(generator.command_generator, 'generate_mountpoint_command') as mock_mountpoint, \
             patch.object(generator.command_generator, 'generate_mount_command') as mock_mount:

            mock_mountpoint.return_value = {"cmd": "mkdir"}
            mock_mount.return_value = {"cmd": "mount"}

            result = generator._generate_mount_commands([mock_device], ["/dev/excluded"])
    ...
    def test_generate_mount_commands_excluded(self, mock_stdout, mock_device):
        ...
        with patch.object(generator.exclusion_checker, 'should_exclude_device', return_value=True):
            result = generator._generate_mount_commands([mock_device], ["/dev/excluded"])

            assert result == []

    def test_generate_device_commands_excluded(self, mock_stdout, mock_device, mock_pyro_config):
        """Test device command generation with excluded device."""
        generator = SketchGenerator(stdout=mock_stdout)

        with patch.object(generator.exclusion_checker, 'should_exclude_device', return_value=True):
            result = generator._generate_device_commands([mock_device], mock_pyro_config.excludes)
    ...
    def test_generate_creation_command_directory(self, mock_stdout):
        ...
        with patch.object(generator.command_generator, 'generate_directory_command') as mock_cmd:
    ...
    def test_generate_creation_command_file(self, mock_stdout):
        ...
        with patch.object(generator.command_generator, 'generate_file_command') as mock_cmd:
    ...
    def test_generate_creation_command_symlink(self, mock_stdout):
        ...
        with patch.object(generator.command_generator, 'generate_symlink_command') as mock_cmd:
...
#       """Test sketch generation with empty config."""
    ...
    def test_complete_configure_workflow(self, mock_stdout, mock_pyro_config):
        ...
        with patch.object(generator.command_generator, 'generate_user_command') as mock_user_cmd, \
             patch.object(generator.command_generator, 'generate_group_command') as mock_group_cmd, \
             patch.object(generator.command_generator, 'generate_mountpoint_command') as mock_mountpoint, \
             patch.object(generator.command_generator, 'generate_mount_command') as mock_mount, \
             patch.object(generator.command_generator, 'generate_directory_command') as mock_dir, \
             patch.object(generator.command_generator, 'generate_file_command') as mock_file, \
             patch.object(generator.command_generator, 'generate_symlink_command') as mock_link, \
             patch.object(generator.command_generator, 'generate_permission_command') as mock_perm:
...
class TestDryRunMode:
    ...
    def test_dry_run_configure_sketch(self, mock_stdout, mock_pyro_config):
        ...
        # Command generator should have dry_run=True
        assert generator.command_generator.dry_run == True
        assert generator.command_generator.cmd_prefix == '# '

        with patch.object(generator.command_generator, 'generate_user_command') as mock_cmd:
            mock_cmd.return_value = {"cmd": "# useradd"}  # Should have prefix

            sketch = generator.generate_configure_sketch(mock_pyro_config)

            # Sketch should be generated normally
    ...
    def test_dry_run_cleanup_commands(self, mock_stdout, mock_pyro_config):
        ...
        with patch.object(generator.exclusion_checker, 'should_exclude_user', return_value=False), \
             patch.object(generator.command_generator, 'generate_cleanup_user_command') as mock_cmd:

            mock_cmd.return_value = {"cmd": "# userdel"}  # Should have prefix

            commands = generator._generate_user_cleanup_commands(mock_pyro_config, compared)
