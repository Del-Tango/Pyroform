...
#       """Test that validation results are not accidentally modified."""
    ...
    def test_validate_configuration_with_empty_config(self, mock_stdout):
        ...
        with patch.object(validator.scanner, 'scan_system_state') as mock_scan, \
             patch.object(validator.comparator, 'compare_states') as mock_compare:
...
class TestLogging:
    ...
    def test_validate_configuration_debug_output(
        ...
        validator = SystemValidator(stdout=mock_stdout, config={"debug": True})

        result = validator.validate_configuration(mock_pyro_config)

        # Should have debug output
        assert mock_stdout.debug.called

        # Check debug message format
        debug_call = mock_stdout.debug.call_args[0][0]
        assert debug_call.startswith("Summary:")

        # Should contain JSON
        import json
        json_str = debug_call.split("Summary: ", 1)[1]
