...
def sample_sketch():
    ...
    return {
        "name": "Test Sketch",
        "steps": [{"name": "Test Step", "cmd": "echo 'test'"}],
    }
...
class TestPyroflowEngineInitialization:
    ...
    def test_init_defaults(self, mock_stdout, mock_flow_config):
        ...
        with patch(
            "pyroform.src.flow_engine.STDOUTMsg", return_value=mock_stdout
        ), patch("pyroform.src.flow_engine.FlowConfig", mock_flow_config), patch(
            "pyroform.src.flow_engine.FlowEngine"
        ) as mock_flow_engine_cls:
    ...
    def test_init_with_stdout(self, mock_stdout, mock_flow_config):
        ...
        with patch("pyroform.src.flow_engine.FlowConfig", mock_flow_config), patch(
            "pyroform.src.flow_engine.FlowEngine"
        ) as mock_flow_engine_cls:
    ...
    def test_init_with_config_path(self, mock_stdout, tmp_path):
        ...
        config_data = {"log_dir": "/custom/log/path", "debug": False, "silence": True}
        ...
        with patch(
            "pyroform.src.flow_engine.STDOUTMsg", return_value=mock_stdout
        ), patch("pyroform.src.flow_engine.FlowConfig") as mock_flow_config_cls, patch(
            "pyroform.src.flow_engine.FlowEngine"
        ) as mock_flow_engine_cls:
    ...
    def test_init_with_yaml_config(self, mock_stdout, tmp_path):
        ...
        config_data = {"log_dir": "/yaml/config/path", "debug": False}
    ...
    def test_init_flow_engine_creation(self, mock_stdout):
        ...
        with patch(
            ...
            mock_stdout.debug.assert_any_call(
                f"flow_engine - {mock_flow_engine_instance}"
            )
...
#   def test_execute_sketch_load_failure(
#       self, mock_stdout, mock_flow_engine, sample_sketch
#   ):
#       """Test sketch execution when loading fails."""
#   def test_execute_sketch_different_actions(
#       self, mock_stdout, mock_flow_engine, sample_sketch
#   ):
...
#       """Test sketch execution with empty sketch."""
    ...
    def test_execute_sketch_without_success_attribute(
        self, mock_stdout, mock_flow_engine, sample_sketch
    ):
        ...
        with patch("builtins.open", mock_open()), patch("json.dump"), patch(
            "pyroform.src.flow_engine.Path"
        ):
    ...
    def test_execute_sketch_exception(
        self, mock_stdout, mock_flow_engine, sample_sketch
    ):
    ...
    def test_execute_sketch_temp_file_cleanup_commented(
        self, mock_stdout, mock_flow_engine, sample_sketch
    ):
        ...
        with patch("builtins.open", mock_open()), patch("json.dump"), patch(
            "pyroform.src.flow_engine.Path", return_value=mock_path
        ):
...
class TestControlMethods:
    ...
    def test_control_methods_without_success_attribute(
        self, mock_stdout, mock_flow_engine
    ):
...
class TestPurgeData:
    ...
    # TODO
#   def test_purge_data_in_execute_sketch(
#       self, mock_stdout, mock_flow_engine, sample_sketch
#   ):
...
class TestCurrentSketchProperty:
    ...
    def test_current_sketch_after_execution(
        self, mock_stdout, mock_flow_engine, sample_sketch
    ):
    ...
    def test_current_sketch_readonly(
        self, mock_stdout, mock_flow_engine, sample_sketch
    ):
...
class TestErrorHandling:
    ...
    def test_execute_sketch_invalid_json(self, mock_stdout, mock_flow_engine):
        ...
        invalid_sketch = {"name": "test", "data": object()}  # Not JSON serializable
    ...
    def test_execute_sketch_file_write_error(
        self, mock_stdout, mock_flow_engine, sample_sketch
    ):
