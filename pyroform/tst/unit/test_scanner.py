...
from pyroform.src.models import PyroConfig, Exclude, SystemUser, SystemGroup, MountedDevice, FileSystemEntry
...
#       """Test default parameter values."""
    ...
    def test_scan_system_state_basic(self, mock_scan_devices, mock_scan_groups,
                                   mock_scan_users, scanner, mock_stdout):
    ...
    @patch.object(SystemStateScanner, '_scan_groups')
    @patch.object(SystemStateScanner, '_scan_mounted_devices')
    def test_scan_system_state_with_config(self, mock_scan_devices, mock_scan_groups,
                                         mock_scan_users, scanner, pyro_config):
    ...
    @patch.object(SystemStateScanner, '_scan_groups')
    @patch.object(SystemStateScanner, '_scan_mounted_devices')
    def test_scan_system_state_debug_output(self, mock_scan_devices, mock_scan_groups,
                                          mock_scan_users, scanner, mock_stdout):
        """Test debug output formatting."""
        system_state = {
            'users': [{'username': 'test'}],
            'groups': [{'groupname': 'test'}],
            'mounted_devices': [{'device': 'test'}]
        }

        mock_scan_users.return_value = system_state['users']
        mock_scan_groups.return_value = system_state['groups']
        mock_scan_devices.return_value = system_state['mounted_devices']

        scanner.scan_system_state()

        # Verify JSON debug output
        mock_stdout.debug.assert_called_with(
            f'System State: {json.dumps(system_state, indent=4)}'
        )
...
#       """Test user exclusion check without excludes."""
    ...
    def test_scan_users_with_groups(self, mock_should_exclude, mock_get_groups,
                                  mock_getpwall, scanner, mock_pwd_user):
    ...
    @patch('pwd.getpwall')
    @patch.object(SystemStateScanner, '_should_exclude_user')
    def test_scan_users_exclusion(self, mock_should_exclude, mock_getpwall,
                                scanner, mock_pwd_user, pyro_config):
        """Test user exclusion."""
        mock_getpwall.return_value = mock_pwd_user
        mock_should_exclude.return_value = True

        result = scanner._scan_users(pyro_config)

        assert len(result) == 0
        scanner.stdout.warn.assert_called_once_with('Excluding user testuser')
...
class TestGroupScanning:
    ...
    @patch('grp.getgrall')
    @patch.object(SystemStateScanner, '_should_exclude_group')
    def test_scan_groups_exclusion(self, mock_should_exclude, mock_getgrall,
                                 scanner, mock_grp_group, pyro_config):
        """Test group exclusion."""
        mock_getgrall.return_value = mock_grp_group
        mock_should_exclude.return_value = True

        result = scanner._scan_groups(pyro_config)

        assert len(result) == 0
        scanner.stdout.warn.assert_called_once_with('Excluding group testgroup')
...
#       """Test df command format parsing."""
    ...
    def test_scan_mounted_devices_empty_config(self, mock_get_mounts, scanner, empty_pyro_config):
    ...
    def test_parse_mount_entry_success(self, mock_should_exclude, mock_get_partitions,
                                     scanner, empty_pyro_config):
    ...
    @patch.object(SystemStateScanner, '_should_exclude_device')
    def test_parse_mount_entry_excluded(self, mock_should_exclude, scanner, pyro_config):
        """Test parsing of excluded device."""
        mock_should_exclude.return_value = True

        device_info = scanner._parse_mount_entry(
            "/dev/excluded /mnt ext4 rw 0 0",
            set(),
            pyro_config
        )

        assert device_info is None
        scanner.stdout.warn.assert_called_once_with('Excluding device /dev/excluded')
...
#       """Test processing a symlink."""
    ...
    @patch('os.path.exists')
    @patch('os.path.isdir')
    @patch.object(SystemStateScanner, '_scan_filesystem_recursive')
    def test_scan_device_contents_error(self, mock_scan_recursive, mock_isdir,
                                      mock_exists, scanner):
        """Test device content scanning with error."""
        mock_exists.return_value = True
        mock_isdir.return_value = True
        mock_scan_recursive.side_effect = Exception("Scan error")

        device_info = Mock(spec=MountedDevice)
        device_info.mountpoint = "/mnt/test"

        scanner._scan_device_contents(device_info, 5, True, None)

        scanner.stdout.err.assert_called_with("  Error scanning /mnt/test: Scan error")

    @patch('os.listdir')
    @patch.object(SystemStateScanner, '_process_filesystem_entry')
    def test_scan_filesystem_recursive_basic(self, mock_process_entry, mock_listdir,
                                           scanner, empty_pyro_config):
    ...
    def test_scan_filesystem_recursive_permission_error(self, mock_listdir, scanner):
        ...
        scanner._scan_filesystem_recursive(
            "/protected", device_info, 5, True, 0, None
        )
...
#       """Test processing special file types (should be ignored)."""
    ...
    def test_debug_output_formatting(self, scanner, mock_stdout):
        """Test debug output formatting."""
        with patch.object(scanner, '_scan_users', return_value=[{"user": "test"}]):
            with patch.object(scanner, '_scan_groups', return_value=[{"group": "test"}]):
                with patch.object(scanner, '_scan_mounted_devices', return_value=[{"device": "test"}]):
                    result = scanner.scan_system_state()

                    # Verify JSON formatting in debug output
                    expected_json = json.dumps(result, indent=4)
                    mock_stdout.debug.assert_called_with(
                        f'System State: {expected_json}'
                    )
...
class TestPerformance:
    ...
    def test_recursive_scan_depth_limit(self, mock_listdir, scanner):
        ...
        with patch.object(scanner, '_scan_filesystem_recursive',
                         side_effect=mock_recursive_side_effect):
            # This should only go 2 levels deep
            scanner._scan_filesystem_recursive(
                "/test", Mock(), 2, True, 0, None
            )
