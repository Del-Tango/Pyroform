"""
Integration Tests: Advanced Features & Validation
"""
...
class TestAdvancedFeatures:
    ...
    def sample_system_state(self):
        ...
        return {
            ...
            "mounts": {"/dev/sda1": "/", "/dev/sdb1": "/mnt/test"},
            "files": {
                "/mnt/test/data": {
                    "owner": "testuser",
                    "group": "testgroup",
    ...
    def sample_pyro_config(self):
        ...
        groups = [Group("desired_group", "testgroup", ["testuser", "newuser"])]
        devices = [
            Device(
                "desired_device",
                "/dev/sdb1",
    ...
    def test_system_validation_integration(
        self, mock_get_state, sample_pyro_config, sample_system_state
    ):
    ...
    def test_scorch_engine_integration(
        self, mock_unlink, mock_remove, mock_get_state, sample_pyro_config
    ):
        ...
        mock_get_state.return_value = {
            ...
            "mounts": {"/dev/sdb1": "/mnt/test", "/dev/sdc1": "/mnt/extra"},
            "files": {
                "/mnt/test/data": {
                    "owner": "testuser",
                    "group": "testgroup",
        ...
        removed_resources = [
            r for r in result.resources_removed if not r.endswith("(dry-run)")
        ]
    ...
    @patch("pyroform.src.scorch_engine.ScorchEngine._get_current_system_state")
    @patch("pyroform.src.scorch_engine.os.remove")
    def test_scorch_rollback_mechanism(
        self, mock_remove, mock_get_state, sample_pyro_config
    ):
    ...
    def test_comprehensive_validation_scenarios(self):
        ...
        for scenario in test_scenarios:
            with patch.object(
                ...
                return_value=scenario["current_state"],
            ):
                # Create minimal PyroConfig for testing
                users = [
                    User(f"user_{u}", u, "pass", [])
                    for u in scenario["desired_state"]["users"]
                ]
                groups = [
                    Group(f"group_{g}", g, [])
                    for g in scenario["desired_state"]["groups"]
                ]
                config = PyroConfig(scenario["name"], users, groups, [])

                result = validator.validate_configuration(config)
                assert (
                    result.is_valid == scenario["should_be_valid"]
