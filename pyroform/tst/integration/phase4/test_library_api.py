"""
Phase 4 Integration Tests: Library API & Integration
"""
import pytest
import tempfile
import json
...
class TestPhase4LibraryAPI:
    ...
    def test_module_level_imports(self):
        ...
        from pyroform import Pyroform
        from pyroform.src.models import PyroConfig, ActionType
    ...
    def test_library_with_different_file_formats(self, tmp_path):
        ...
        with patch('pyroform.pyroform.PyroformEngine') as MockEngine:
            ...
            with open(json_config, 'w') as f:
                json.dump({"Label": "JSON Test", "Users": [], "Groups": [], "Devices": []}, f)
...
#           pf.configure(str(sample_pyro_file), **kwargs)
#           pf.engine.configure.assert_called_with(str(sample_pyro_file), **kwargs)
...
#               try:
#                   pf = Pyroform(auto_confirm=True)
#                   # Mock the engine to avoid actual system changes
#                   with patch.object(pf.engine, 'configure', return_value=True):
#                       result = pf.configure(f"/fake/config_{worker_id}.yaml")
#                       results_list.append((worker_id, result))
#               except Exception as e:
#                   results_list.append((worker_id, str(e)))
...
#           # If methods return self, they can be chained
#           # This depends on the actual implementation
#           with patch.object(pf.engine, 'configure', return_value=True):
#               with patch.object(pf.engine, 'mount', return_value=True):
#                   # Example of potential method chaining
#                   result = pf.configure("/path/config.yaml").mount("/path/config.yaml")
#                   # This would require methods to return self
...
#       def mock_pyroform_engine(self):
#           """Mock the internal Pyroform engine"""
#           with patch('pyroform.PyroformEngine') as mock_engine_class:
#               mock_engine = Mock()
#               mock_engine.configure.return_value = True
#               mock_engine.scorch.return_value = True
#               mock_engine.mount.return_value = True
...
#           """Test library can be used in different contexts"""
#           # Simulate usage in another script
#           with patch('pyroform.PyroformEngine') as mock_engine:
#               mock_engine.return_value.configure.return_value = True
...
#           # Both should use the same configuration
#           assert pf1.engine.config.safety_checks is True
#           assert pf2.engine.config.safety_checks is True
