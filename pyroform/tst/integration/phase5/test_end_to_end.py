...
class TestPhase5EndToEnd:
    ...
    def test_complete_cli_workflow(self, mock_get_state, mock_flow_engine, temp_config_environment):
        ...
        mock_flow_instance.start_procedure.return_value = Mock(success=True, message="Success")
        ...
        result = runner.invoke(cli, [
            ...
            '-C',
            '-i', str(temp_config_environment['pyro_dir']),
            '-c', str(temp_config_environment['pyroform_config']),
            '-y',  # Auto-confirm
            '-r'   # Generate report
        ])
    ...
    def test_complete_library_workflow(self, mock_engine_class, temp_config_environment):
        ...
        assert final_validation.is_valid is True, f"Expected True but got {final_validation.is_valid}. Validation calls: {mock_engine.validate.call_count}"
    ...
    def test_scorch_workflow(self, mock_get_state, temp_config_environment):
        ...
        mock_get_state.return_value = {
            ...
            "files": {
                "/mnt/data/app": {"owner": "appuser", "group": "appgroup", "perms": "750"},
                "/mnt/old/data": {"owner": "olduser", "group": "oldgroup", "perms": "755"}
            }
        }
    ...
    def test_multi_config_workflow(self, temp_config_environment):
        ...
        with patch.object(pf.engine, 'configure') as mock_configure:
            ...
            mock_configure.assert_called_with(str(temp_config_environment['pyro_dir']), auto_confirm=True)
    ...
    def test_validation_workflow(self, mock_get_state, temp_config_environment):
        ...
        mock_get_state.return_value = {
            ...
            "files": {
                "/mnt/data/app": {"owner": "appuser", "group": "appgroup", "perms": "750"},
                "/mnt/data/shared": {"owner": "datauser", "group": "datagroup", "perms": "775"},
                "/mnt/data/app/config.yaml": {"owner": "appuser", "group": "appgroup", "perms": "600"},
                "/mnt/data/shared/data.txt": {"owner": "datauser", "group": "datagroup", "perms": "664"}
            }
        }
    ...
    def test_report_generation_workflow(self, temp_config_environment):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json') as report_file:
            with patch('pyroform.src.flow_engine.FlowEngine') as mock_flow_engine:
                ...
                mock_instance.start_procedure.return_value = Mock(success=True, message="Success")
                mock_flow_engine.return_value = mock_instance

                result = runner.invoke(cli, [
                    'main',  # Specify the main command
                    '-C',
                    '-i', str(temp_config_environment['main_config']),
                    '-o', report_file.name,
                    '-r',
                    '-y'
                ])
    ...
    def test_comprehensive_error_workflow(self, temp_config_environment):
        ...
        with patch.object(pf.engine, 'configure') as mock_configure:
            with patch.object(pf.engine, 'mount') as mock_mount:
                ...
                configure_result = pf.configure(str(temp_config_environment['main_config']))
...
#           mock_engine.configure.return_value = True
#           mock_engine.mount.return_value = True
#           mock_engine.validate.return_value = Mock(is_valid=False, discrepancies=[{"type": "user", "issue": "test"}])
#           mock_engine.scorch.return_value = Mock(resources_removed=[], resources_failed=[])
#           mock_engine_class.return_value = mock_engine
...
#           # 1. Validate current state
#           validation = pf.validate(config_file)
#           assert validation.is_valid is False  # We mocked it to return false
...
#           # 4. Validate again (should be valid now)
#           mock_engine.validate.return_value = Mock(is_valid=True, discrepancies=[])
#           final_validation = pf.validate(config_file)
...
#               assert result is True
#               # Verify engine was called with directory path
#               mock_configure.assert_called_with(str(temp_config_environment['pyro_dir']))
...
#   """
#   pytest.ini for running the integration test suite:
#   ```ini
#   [tool:pytest]
#   testpaths = pyroform/tst/integration
...
#       phase2: Phase 2 FlowCTRL Integration Tests
#       phase3: Phase 3 Advanced Features Tests
#       phase4: Phase 4 Library API Tests
#       phase5: Phase 5 End-to-End Tests
