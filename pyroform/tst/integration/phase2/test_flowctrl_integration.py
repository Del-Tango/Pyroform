"""
Phase 2 Integration Tests: FlowCTRL Integration & Basic Actions
"""
import pytest
import json
import tempfile
...
class TestPhase2FlowIntegration:
    ...
    def sample_pyro_config(self):
        ...
        users = [
            User("test_user", "testuser", "testpass123", ["testgroup"])
        ]
        groups = [
            Group("test_group", "testgroup", ["testuser"])
        ]
        devices = [
            Device(
                "test_device",
    ...
    def test_error_handling_in_flow_execution(self, sample_pyro_config, mock_flow_engine):
...
#           """Mock FlowEngine for testing"""
#           with patch('pyroform.src.flow_engine.FlowEngine') as mock_flow:
#               mock_instance = Mock(spec=FlowEngine)
#               mock_instance.load_procedure.return_value = True
#               mock_instance.start_procedure.return_value = Mock(
#                   success=True,
#                   message="Procedure completed successfully"
...
#           device_cmd = device_commands[0]
#           assert "test_device" in device_cmd["name"]
#           assert "mount" in device_cmd["cmd"] or "mkdir" in device_cmd["setup-cmd"]
#           assert "/dev/sdb1" in str(device_cmd.values())
#           assert "/mnt/test" in str(device_cmd.values())
