...
class ReportGenerator:
    ...
    def generate_validation_report(self, validation_result: Any) -> Dict[str, Any]:
        ...
        report["warnings"] = len([d for d in discrepancies if not d.get('critical', False)])

        return report

    def save_report(self, report: Dict[str, Any], output_path: Path, format: str = "json") -> bool:
        """
        Save report to file
    ...
    def _generate_summary(self, action: str, result: Any) -> Dict[str, Any]:
        ...
        if hasattr(result, '__class__') and result.__class__.__name__ == 'ScorchResult':
            ...
            summary.update({
                ...
                "total_issues": getattr(result, 'summary', {}).get('total_issues', 0),
                "critical_issues": getattr(result, 'summary', {}).get('critical_issues', 0)
            })
        else:
    ...
    def _generate_combined_summary(self, reports: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
...
#           elif action == "mount":
#               summary.update({
#                   "devices_processed": getattr(result, 'devices_processed', 0),
#                   "mounts_created": getattr(result, 'mounts_created', 0),
#                   "errors": len(getattr(result, 'errors', []))
#               })
