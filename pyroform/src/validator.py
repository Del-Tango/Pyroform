...
class SystemValidator:
    ...
    def get_validation_report(self) -> Dict[str, Any]:
        ...
        passed = total_checks - failed  # This is simplified - in reality we'd count actual checks
    ...
    def _compare_states(self, config: PyroConfig, current_state: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        ...
        for device in config.devices:
            for state_entry in device.state:
                ...
                if len(state_parts) >= 5:
                    obj_type, path, expected_owner, expected_group, expected_perms = state_parts[:5]
                    ...
                    if path not in current_files:
                        discrepancies.append({
                            ...
                            "critical": obj_type == "dir"  # Missing dir is critical, missing file is not
                        })
                    else:
...
#               Comprehensive validation report
#           """
#           # This would typically use the last validation result
...
#           # Check mounts
#           current_mounts = current_state["mounts"]
#           for device in config.devices:
#               if device.path not in current_mounts:
#                   discrepancies.append({
#                       "type": "mount",
