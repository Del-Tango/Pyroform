...
#       print(f'[ DEBUG ]: resources failed - {failed}')
    ...
    def _discover_filesystem_state(self) -> Dict[str, List[str]]:
        ...
        """
        filesystem_state = {"files": [], "directories": [], "links": []}

        # Define managed locations (these should be configurable)
        managed_locations = ["/home", "/mnt", "/opt", "/var/lib"]  # Add more as needed
    ...
    def _calculate_cleanup_set(
        ...
        for mount in mounts_to_remove.copy():
            if mount in system_mounts or any(
                mount.startswith(sys_mount + "/") for sys_mount in system_mounts
            ):
    ...
    def _execute_removal(self, resource_type: str, resource_id: str) -> bool:
        ...
        try:
            if resource_type == "user":
                ...
                result = subprocess.run(
                    ["which", "userdel"], capture_output=True, text=True, check=False
                )
                ...
                result = subprocess.run(
                    ["which", "groupdel"], capture_output=True, text=True, check=False
                )
                if result.returncode != 0:
                    return False
                ...
                result = subprocess.run(
                    ["which", "umount"], capture_output=True, text=True, check=False
                )
                if result.returncode != 0:
                    return False

                result = subprocess.run(
                    ["umount", resource_id], capture_output=True, text=True, check=False
                )
                return result.returncode == 0
