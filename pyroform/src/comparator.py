...
class SystemStateComparator:
    ...
    # @pysnooper.snoop()
    def compare_states(
        self, system_state: Dict[str, Any], config: PyroConfig
    ) -> Dict[str, Any]:
        """
        ...
        if config.users:
            self._compare_users(
                config.users, system_state["users"], differences, config.excludes
            )

        if config.groups:
            self._compare_groups(
                config.groups, system_state["groups"], differences, config.excludes
            )

        if config.devices:
            current_fs_state = self._build_filesystem_state(system_state)
            self._compare_filesystem(
                config.devices, current_fs_state, differences, config.excludes
            )
    ...
    def _build_filesystem_state(
        ...
        for device in system_state["mounted_devices"]:
            ...
            self._add_filesystem_entries(
                device["directories"], "directory", mountpoint, fs_state
            )
            self._add_filesystem_entries(device["files"], "file", mountpoint, fs_state)
            self._add_filesystem_entries(
                device["symlinks"], "symlink", mountpoint, fs_state
            )
    ...
    def _compare_users(
        ...
        self._find_extra_users(
            config_user_names, current_users_dict, differences, excludes
        )

    # @pysnooper.snoop()
    def _check_user_compliance(
        self, config_user: User, current_users: Dict, differences: Dict[str, Any]
    ) -> None:
        ...
        if username not in current_users:
            ...
            self._check_user_properties(
                config_user, current_users[username], differences
            )

    # @pysnooper.snoop()
    def _check_user_properties(
        self, config_user: User, current_user: Dict, differences: Dict[str, Any]
    ) -> None:
        ...
        extra_groups = [
            item
            for item in current_groups - expected_groups
    ...
    def _compare_groups(
        ...
        self._find_extra_groups(
            config_group_names, current_groups_dict, differences, excludes
        )

    def _check_group_compliance(
        self, config_group: Group, current_groups: Dict, differences: Dict[str, Any]
    ) -> None:
        ...
        if groupname not in current_groups:
            ...
            self._check_group_properties(
                config_group, current_groups[groupname], differences
            )

    def _check_group_properties(
        self, config_group: Group, current_group: Dict, differences: Dict[str, Any]
    ) -> None:
    ...
    def _compare_filesystem(
        ...
        for device in config_devices:
            ...
            self._check_device_contents(
                device, current_fs_state, differences, config_paths, excludes
            )
    ...
    def _check_mountpoint(
        self, device: Device, current_fs_state: Dict, differences: Dict[str, Any]
    ) -> None:
        ...
        if not mountpoint_exists:
            differences["missing_mountpoints"].append(
                {"label": device.label, "mountpoint": device.mountpoint}
            )
    ...
    def _check_device_contents(
        ...
        for state_entry in device.state:
            ...
            self._check_filesystem_item_compliance(
    ...
    def _check_filesystem_item_compliance(
        ...
        if path not in current_fs_state:
            ...
            self._check_item_properties(
                config_item, current_fs_state[path], differences
            )

    def _handle_missing_item(
        self, config_item: Dict[str, Any], differences: Dict[str, Any]
    ) -> None:
    ...
    def _find_property_mismatches(
        self, config_item: Dict[str, Any], current_item: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        if (
            config_item["type"] == "ln"
            and current_item["type"] == "symlink"
            and "target" in config_item
            and config_item["target"] != current_item.get("target")
        ):
            mismatches.append(
                {
                    "property": "target",
    ...
    def _should_exclude(
        self, item: str, excludes: Optional[Exclude], exclude_type: str
    ) -> bool:
