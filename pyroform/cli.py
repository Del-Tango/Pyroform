...
#   class BannerCommand(click.Command):
#       def invoke(self, ctx):
#           # Display banner for all commands (except when help is explicitly called)
...
#   class BannerGroup(click.Group):
#       def invoke(self, ctx):
#           # Display banner for all group commands
#           if not ctx.protected_args or '--help' not in ctx.protected_args:
#               self._display_banner()
#           return super().invoke(ctx)
...
#   """
#           formatter.write(banner_text)
#           # Then call the parent to show normal usage
#           super().format_usage(ctx, formatter)
